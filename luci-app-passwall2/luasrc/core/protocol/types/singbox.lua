module("luci.passwall2.core.protocol.types.singbox", package.seeall)

local base = require "luci.passwall2.core.protocol.base"

-- Sing-box protocol implementation
local singbox = base:new({
    name = "Sing-box",
    type = "sing-box",
    protocols = {
        "vmess", "vless", "shadowsocks", "shadowsocksr", 
        "trojan", "wireguard", "hysteria", "hysteria2",
        "tuic", "anytls", "ssh"
    },
    config_schema = {
        -- Common fields
...
function singbox:validate_config(config)
    local errors = {}
...
function singbox:generate_config(config)
    ...
    local generated_config = {
        ...
        tag = config.remarks or "singbox-node"
    }
...
-- Test Sing-box connection
function singbox:test_connection(config)
...
-- Get Sing-box specific configuration options for UI
function singbox:get_ui_options()
    return {
        protocol_options = {
            vmess = {
...
function singbox:get_protocol_display_info(protocol)
    local display_info = {
        vmess = { title = "VMess", description = "VMess protocol for Sing-box" },
        vless = { title = "VLESS", description = "VLESS protocol for Sing-box" },
